...
export class databaseService {
  static async createPlayer(user: User) {
    const query = `INSERT INTO players (name, elo, id_discord)
                       VALUES ('${user.username}', 1000, '${user.id}')`;
    try {
      ...
      console.log(
        `Player ${saved.name} saved on database correctly with id ${saved.id}`
      );
      return saved;
    } catch (error) {
      console.error(error);
  ...
  static async createTeamPlayer(player: any) {
    const query = `INSERT INTO team_players (id_team, id_role, id_player)
                       VALUES ('${player.id_team}', '${player.id_role}', '${player.id_player}')`;
    try {
      const [result] = await pool.query(query);
      const { insertId } = result as ResultSetHeader;
      return insertId;
  ...
  static async createTeam(players: Player[]) {
    let teamElo = players.reduce((acc, player) => {
      return acc + player.elo;
    }, 0);
    teamElo = teamElo / 5;
    const query = `INSERT INTO teams (elo)
                       VALUES ('${teamElo}')`;
    try {
      const [result] = await pool.query(query);
      const { insertId } = result as ResultSetHeader;
  ...
  static async createMatch(players: Player[]) {
    ...
    const query = `INSERT INTO matches (id_team_blue, id_team_red, id_team_winner)
                       VALUES ('${blueTeamId}', '${redTeamId}', '${redTeamId}')`;
    try {
      const [result] = await pool.query(query);
      const { insertId } = result as ResultSetHeader;
