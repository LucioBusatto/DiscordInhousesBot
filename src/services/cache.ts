...
class Cache<TKey extends unknown, TValue extends unknown> {
  ...
  constructor(
    ttl: number,
    onDeleteCallback?: CacheDeleteCallback<TKey, TValue>
  ) {
    this.ttl = ttl;
    if (onDeleteCallback) {
  ...
  private clearEntries() {
    ...
    this.cache.forEach((entry, key) => {
      const hasExpired =
        now.getTime() - entry.createdAt.getTime() > this.ttl * 60 * 1000;
      if (hasExpired) {
...
export const matchesCache = new Cache<string, Player[]>(1);
